...
class NodeSync extends EventEmitter {
  constructor () {
    ...
    node.on('state', _.bind(this._onNodeStateChanged, this))
  }
  ...
  start () {
    ...
    this._syncPromise = Q.try(() => {
      if (!(node.state === node.STATES.CONNECTED)) {
        throw new Error('Cannot sync - plume node not yet connected')
      }
  ...
  _onNodeStateChanged (state) {
    switch (state) { // eslint-disable-line default-case
      ...
      case node.STATES.STOPPING:
        log.info('plume node stopping, so stop sync')

        this.stop()
        break
      // auto-sync whenever node gets connected
      case node.STATES.CONNECTED:
        log.info('plume node connected, re-start sync')
