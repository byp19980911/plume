  ...
  _updateOwnedTransaction (txHash, block) {
    ...
    this.web3.eth
      ...
      .then(receipt => {
        ...
        if (tx != null) {
          log.info('Update owned transaction on db: \n', _.pick(tx, ['hash', 'blockNumber', 'timestamp']))
          log.info('Update owned transaction on chain: \n', _.pick(transaction, ['hash', 'blockNumber']))
    ...
    this._getAccounts()
      .then(accounts => {
        return accounts.filter(
          account => {
            if (tx.from && account.toLowerCase() === tx.from.toLowerCase()) {
              return true
            }
            if (tx.to && account.toLowerCase === tx.to.toLowerCase()) {
              return true
            }
          }
        )
