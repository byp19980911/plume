...
class ContractState extends EventEmitter {
  constructor () {
    ...
    ipc.on(Types.EXECUTE_CONTRACT, _.bind(this._sendTransaction, this))
  }
  ...
  _deploy (data) {
    ...
    Promise.all([
      ...
      web3.eth.getTransactionCount(data.from, 'pending')
    ])
      .then(([gas, price, transactionCount]) => {
        let inputData = tx.encodeABI()
        let chainId = settings.chainId
        let rawTx = {
          'from': data.from,
          'gasPrice': new BigNumber(price).toString(),
        ...
        if (data.value && this._checkValue(data.value)) {
          ...
          rawTx.value = web3.utils.toWei(value)
        }
        rawTx['nonce'] = transactionCount.toString()
        log.info(rawTx)
        global.rpc.signTx(rawTx).then(res => {
          web3.eth
          ...
          if (error) {
            ...
            global.windows.broadcast(Types.DEPLOY_CONTRACT_REPLY, reply)
          }
        })
      }).catch(error => {
    ...
    try {
      global.rpc.signTx(rawTx).then(res => {
        web3.eth
          ...
          .on('error', error => {
            log.info(error)
            global.windows.broadcast(Types.EXECUTE_CONTRACT_REPLY, {msg: error.message, error: true})
          })
          .once('transactionHash', (txHash) => {
            log.info(txHash)
            global.windows.broadcast(Types.EXECUTE_CONTRACT_REPLY, {msg: 'Send transaction hash: ' + txHash})
          })
          .on('receipt', (receipt) => {
            let msg = ''
            if (receipt.status === '0x1' || receipt.status === true) {
              msg = 'contract execute success'
              global.windows.broadcast(Types.EXECUTE_CONTRACT_REPLY, {msg})
            } else {
              msg = 'contract execute fail'
              global.windows.broadcast(Types.EXECUTE_CONTRACT_REPLY, {msg, error: true})
            }
          })
      }).catch(error => {
        if (error) {
          global.windows.broadcast(Types.EXECUTE_CONTRACT_REPLY, {msg: 'wrong-passwd', error: true})
        }
      })
    } catch (e) {
      log.info(e)
      global.windows.broadcast(Types.EXECUTE_CONTRACT_REPLY, {msg: e.message, error: true})
    }
  }
  ...
  _parseArguments (abi, args) {
    ...
    return cons.inputs.map((input, idx) => {
      if (input.type.indexOf('uint') > -1) {
        let num = new BigNumber(args[idx])
  ...
  _callContract (data) {
    const web3 = global.web3
    let { abi, address, method, inputs } = data
    abi = JSON.parse(abi)
