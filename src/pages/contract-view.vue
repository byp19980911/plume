...
export default {
  ...
  methods: {
    ...
    callContract (name) {
      ...
      if (method.constant) {
        try {
          ...
          this.addMessage(e.message, true)
        }
      } else {
    ...
    submit () {
      let chainId = this.$settings.chainId
      let rawTx = {
        'from': this.from,
        'to': this.contract.contractAddress,
        'gasPrice': new BigNumber(this.price).toString(),
        'gas': new BigNumber(this.gas).toString(),
        'data': this.execMethod.encodeABI(),
        'chainId': new BigNumber(chainId).toString(),
        'passwd': this.password
      }
      this.$q.loading.show()
      web3.eth.getTransactionCount(this.from, 'pending').then(transactionCount => {
        rawTx['nonce'] = transactionCount.toString()
        ipc.send(Types.EXECUTE_CONTRACT, {rawTx})
      })
      this.cancel()
    },
  ...
  created () {
    ...
    ipc.on(Types.EXECUTE_CONTRACT_REPLY, (event, data) => {
      this.$q.loading.hide()
      if (data.error) {
        if (data.msg === 'wrong-passwd') {
  ...
  destroyed () {
    ipc.removeAllListeners(Types.EXECUTE_CONTRACT_REPLY)
  }
}
</script>
