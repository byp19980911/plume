<template>
 <div class="trans-container q-item-link" @click="viewTransaction(item)">
   ...
   <div :style="{color: item.receipt.status === '0x1' || item.receipt.status === true ? 'green' : 'red' }">
   ...
   <div>
     <div v-if="address===undefined" color="negative">
       {{toPDX(item.value)}} {{$unit}}
     </div>
     <div class="trans-fee" v-else-if="address.toLowerCase()===item.from.toLowerCase()">
       -{{toPDX(item.value)}} {{$unit}}
     </div>
     <div class="trans-fee" v-else-if="item.to && address.toLowerCase()===item.to.toLowerCase()">
       +{{toPDX(item.value)}} {{$unit}}
     </div>
   </div>
  ...
  methods: {
    ...
    viewTransaction (item) {
      ...
      if (!this.item.receipt) {
        ...
        this.gasFee = this.toPDX(this.item.receipt.gasUsed * this.item.gasPrice)
      }
    },
